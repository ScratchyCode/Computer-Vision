...
# setup per usare la gpu
config = tf.compat.v1.ConfigProto(device_count={'GPU':1,'CPU':1})
#sessione = tf.compat.v1.Session()(config=config)
#sessione = tf.compat.v1.Session(graph=tf.compat.v1.get_default_graph(),config=session_conf)
sessione = tf.compat.v1.Session(graph=tf.compat.v1.get_default_graph(),config=config)
keras.backend.set_session(sessione)
